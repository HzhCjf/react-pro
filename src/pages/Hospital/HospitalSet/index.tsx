...
import { Button, Form, Input, Table, message, Tooltip, Popconfirm } from "antd";
import {
  SearchOutlined,
  EditOutlined,
  DeleteOutlined,
...
import type { typeFormSearchDate } from "./types";
...
export default function HospitalSet() {
  ...
  const pageChange = (page: number, limit: number) => {
    //解决antd警告
    //每次改变页码的时候,清除table数据,反正下边都要重新请求的
    setHospitalList([]);

    //拿到参数的 新的页码 和新的 pageSize
    setRequestParams({ ...requestParams, page, limit });
  };

  //方法2:form表单的提交事件
  const searchFinish = (values: typeFormSearchDate) => {
    //onFinish事件函数默认接受一个参数,就是当前表单收集的值
    console.log(values);
    //可以通过useForm Hook获取form表单的实例,通过实例的getFieldsValue方法获取form的收集的数据
    // console.log(form.getFieldsValue());

    //改变保存请求参数的 状态，从而重新发送请求(需要把page重新设置为1,否则可能会查询不到数据)
    setRequestParams({ ...requestParams, ...values, page: 1 });
  };
  ...
  //方法6:表格中收集选中的id下标的方法
  const selectionChange = (newSelectedRowKeys: React.Key[]) => {
    // console.log(newSelectedRowKeys, "newSelectedRowKeys");
    setIdList(newSelectedRowKeys);
  };
  ...
  return (
    <div>
      <Form layout="inline" form={form} onFinish={searchFinish}>
